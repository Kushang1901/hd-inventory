...
const ROOM_TYPES = [
  { id: "Standard", label: "Standard Room", total: 2, icon: Home, color: "blue" },
  { id: "Deluxe", label: "Deluxe Room", total: 31, icon: BedDouble, color: "amber" },
  { id: "Super Deluxe", label: "Super Deluxe Room", total: 8, icon: Sparkles, color: "purple" },
  { id: "Suite", label: "Suite Room", total: 2, icon: Crown, color: "emerald" },
];
...
export default function RoomRestrictionsPage() {
  ...
  useEffect(() => {
    let activeSFp: any = null;
    let activeEFp: any = null;

    import("flatpickr").then((module) => {
      const fp = module.default;
      import("flatpickr/dist/themes/dark.css");

      activeSFp = fp("#rr-startDate", {
        altInput: true,
        altFormat: "d/m/Y",
        dateFormat: "Y-m-d",
        allowInput: true,
        ...
        onChange: (selectedDates: any[], dateStr: string) => {
          ...
          if (activeEFp && selectedDates.length > 0) {
            activeEFp.set("minDate", selectedDates[0]);
          }
        },
      });

      activeEFp = fp("#rr-endDate", {
        altInput: true,
        altFormat: "d/m/Y",
        dateFormat: "Y-m-d",
        allowInput: true,
      ...
      setStartFp(activeSFp);
      setEndFp(activeEFp);
    });

    return () => {
      if (activeSFp) activeSFp.destroy();
      if (activeEFp) activeEFp.destroy();
    };
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div>
        ...
        <p className="text-xs md:text-sm text-zinc-500 mt-1">
          Partially block rooms of a specific type on available dates — e.g. only show 16 of 31 Deluxe rooms as bookable.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {ROOM_TYPES.map((rt) => {
          ...
          return (
            <div
              key={rt.id}
              className="rounded-xl border border-zinc-800 bg-zinc-900/20 p-4 flex items-center gap-3"
            >
              ...
              <div>
                ...
                {activeCount > 0 && (
                  <p className={`text-[9px] font-semibold ${colors.accent}`}>{activeCount} restriction{activeCount > 1 ? "s" : ""} active</p>
                )}
              </div>
            </div>
          );
                ...
                {ROOM_TYPES.map((rt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 rounded-lg border px-3 py-2.5 text-xs font-semibold transition cursor-pointer text-left ${
                        isSelected
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1">
                  <button
                    type="button"
                    onClick={() => setBlockedCount(Math.max(1, blockedCount - 1))}
                    className="h-7 w-7 rounded-lg border border-zinc-700 bg-zinc-900 text-zinc-300 flex items-center justify-center text-sm hover:bg-zinc-800 transition cursor-pointer font-bold"
                  >−</button>
                  <input
                    type="number"
                    min={1}
                  ...
                  <button
                    type="button"
                    onClick={() => setBlockedCount(Math.min(maxBlock, blockedCount + 1))}
                    className="h-7 w-7 rounded-lg border border-zinc-700 bg-zinc-900 text-zinc-300 flex items-center justify-center text-sm hover:bg-zinc-800 transition cursor-pointer font-bold"
                  >+</button>
                </div>
              </div>
                        ...
                        >
                          ...
                          <td className="px-5 py-4 text-right">
                            <div className="flex justify-end gap-1">
                              <button
                                onClick={(e) => { e.stopPropagation(); startEditFromRow(r); }}
              ...
              <div className="space-y-2">
                ...
                <div className="rounded-xl border border-zinc-800 bg-zinc-900/20 p-4 space-y-3">
                  ...
                  <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${ROOM_COLORS[selectedPreview.roomType]?.bar}`}
              ...
              <div className="space-y-2">
                ...
                <div className="rounded-xl border border-amber-500/10 bg-amber-500/5 p-4 italic text-zinc-300 relative min-h-[52px]">
                  ...
                  <p className="pl-4 py-1 text-zinc-300 leading-relaxed font-serif text-sm">
                    {selectedPreview.reason || "No specific reason was provided for this restriction."}
                  </p>
                </div>
              </div>
